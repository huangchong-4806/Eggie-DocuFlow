...
Eggie Excel Tool 是一个面向日常办公场景的 Excel 合并拆分工具。当前版本同时支持
Excel 合并和 Excel 拆分，适合需要按文件顺序汇总多份表格，或按固定数据行数拆分
大表格的用户。
...
Eggie Excel Tool is a desktop utility for everyday office workflows. The tool
now supports both Excel merging and Excel splitting. It is designed for users
who need to combine multiple Excel workbooks in a specific file order, or split
large Excel files by a custom number of data rows.
